...
async def handle_metadata(bot: Client, message: Message):
    ...
    return await message.reply_text(f"**Oooh~ Check out your current metadata, cutie! 😘 :-**\n\n➜ `{user_metadata}` ",quote=True, reply_markup=InlineKeyboardMarkup(OFF))
...
async def query_metadata(bot: Client, query: CallbackQuery):
    ...
    if data.startswith('metadata_'):
        ...
        if bool(eval(_bool)):
            ...
            await query.message.edit(f"**Hey babe, here’s your current metadata just for you! Wanna make it even sexier? 😉 :-**\n\n➜ `{user_metadata}` ", reply_markup=InlineKeyboardMarkup(OFF))

        else:
            await jishubotz.set_metadata(query.from_user.id, bool_meta=True)
            await query.message.edit(f"**Hey cutie, here’s your current metadata just for you! Wanna spice it up? 😉 :-**\n\n➜ `{user_metadata}` ", reply_markup=InlineKeyboardMarkup(ON))
